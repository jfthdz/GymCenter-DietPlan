...
module.exports = function(appLogin){
    ...
    appLogin.post("/api/login", async function(req, res){
        try {
            ...
            let user = await model.findUserLogin(userName, pass);
    ...
    appLogin.post("/api/findByUserName", async function(req, res){
        try {
            const userName = req.body.userName;

            let user = await model.findUserByUserName(userName);
